...
public interface IYetkiliService
{
    ...
    Task<List<AdminApplicationListItemDto>> GetApplicationsAsync(Guid yetkiliId, CancellationToken cancellationToken);
}
...
public class YetkiliService(AppDbContext db, UserManager<AppUser> userManager, IAccommodationService accommodationService) : IYetkiliService
{
    ...
    public async Task<List<AdminApplicationListItemDto>> GetApplicationsAsync(Guid yetkiliId, CancellationToken cancellationToken)
    {
        var dormitoryIds = await GetAssignedDormitoryIdsAsync(yetkiliId, cancellationToken);
        if (dormitoryIds.Count == 0) return new();
        var query = db.Applications.AsNoTracking().Include(x => x.User).Where(x => x.AccommodationType == AccommodationType.Yurt);
        return await query.OrderByDescending(x => x.CreatedAt)
            .Select(x => new AdminApplicationListItemDto(x.Id, x.UserId, x.User.FullName, x.User.TcNo, x.User.StudentStaffNo, x.AccommodationType, x.DocumentUrl, x.Status, x.CreatedAt))
            .ToListAsync(cancellationToken);
    }
    ...
    public async Task<AdminRoomListItemDto> UpdateRoomAsync(Guid yetkiliId, int roomId, YetkiliRoomUpdateRequest request, CancellationToken cancellationToken)
    {
        ...
        if (priceChanged)
        {
            await UpdateFuturePaymentsForRoomAsync(roomId, request.Price, cancellationToken);
        }

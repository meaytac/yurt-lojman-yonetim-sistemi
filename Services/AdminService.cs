...
public interface IAdminService
{
    ...
    public class AdminService(AppDbContext db, UserManager<AppUser> userManager) : IAdminService
    {
        ...
        public async Task<AdminDashboardStatsDto> GetDashboardStatsAsync(FacilityScope? scope, CancellationToken cancellationToken)
        {
            ...
            List<int>? scopedRoomIds = null;
            if (scope != null)
            {
                scopedRoomIds = await ScopedRooms(scope).Select(x => x.Id).ToListAsync(cancellationToken);
            }
            ...
            var recentRequestsQuery = db.Requests.AsNoTracking().Include(x => x.User).Include(x => x.Room).AsQueryable();
            if (scopedRoomIds != null)
            {
                recentRequestsQuery = recentRequestsQuery.Where(x => scopedRoomIds.Contains(x.RoomId));
            }
            ...
            if (scopedRoomIds != null)
            {
                scopedUserIds = (await db.Placements.AsNoTracking()
                    .Where(x => x.IsActive && scopedRoomIds.Contains(x.RoomId))
                    .Select(x => x.UserId)
                    .ToListAsync(cancellationToken)).ToHashSet();
            }

            var unpaidDebtQuery = db.Payments.AsQueryable();
            if (scopedUserIds != null)
            {
                unpaidDebtQuery = unpaidDebtQuery.Where(x => scopedUserIds.Contains(x.UserId));
            }

            var unpaidDebt = await unpaidDebtQuery
            ...
            return new AdminDashboardStatsDto(
                ...
                OpenRequestCount: scopedRoomIds == null
                    ? await db.Requests.CountAsync(x => x.Status == RequestStatus.Open || x.Status == RequestStatus.InProgress, cancellationToken)
                    : await db.Requests.CountAsync(x => scopedRoomIds.Contains(x.RoomId) && (x.Status == RequestStatus.Open || x.Status == RequestStatus.InProgress), cancellationToken),
        ...
        public async Task<AdminRoomOccupantsResponse> GetRoomOccupantsAsync(int roomId, FacilityScope? scope, CancellationToken cancellationToken)
        {
            ...
            if (scope != null)
            {
                var scopedIds = await ScopedRooms(scope).Select(x => x.Id).ToListAsync(cancellationToken);
                if (!scopedIds.Contains(roomId))
                {
                    throw new KeyNotFoundException("Oda bulunamadi.");
                }
            }
        ...
        public async Task<AdminPagedResponse<AdminUserListItemDto>> GetUsersAsync(AdminUserQuery query, FacilityScope? scope, CancellationToken cancellationToken)
        {
            ...
            if (scope != null)
            {
                var scopedRoomIdsForUsers = await ScopedRooms(scope).Select(x => x.Id).ToListAsync(cancellationToken);
                var scopedUserIdsForUsers = await db.Placements.AsNoTracking()
                    .Where(x => x.IsActive && scopedRoomIdsForUsers.Contains(x.RoomId))
                    .Select(x => x.UserId)
                    .ToListAsync(cancellationToken);
                var scopedUserIdSet = scopedUserIdsForUsers.ToHashSet();
                users = users.Where(x => scopedUserIdSet.Contains(x.Id));
            }
    ...
    public async Task<IReadOnlyList<AdminPlacementListItemDto>> GetPlacementsAsync(bool activeOnly, FacilityScope? scope, CancellationToken cancellationToken)
        ...
        return await query.OrderByDescending(x => x.CheckInDate)
            .Select(x => new AdminPlacementListItemDto(
                x.Id,
                x.UserId,
                x.User.FullName,
                x.RoomId,

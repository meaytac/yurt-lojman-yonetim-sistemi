    ...
    IAccommodationService accommodationService) : ControllerBase
{
    ...
    public async Task<IActionResult> AssignApplication(int id, ApplicationDecisionRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.RoomId.HasValue)
        {
            ...
            var roomForType = await db.Rooms.AsNoTracking().Include(x => x.BlockFloor).ThenInclude(x => x.Building).FirstOrDefaultAsync(x => x.Id == request.RoomId.Value, cancellationToken);
            if (roomForType != null)
            {
                var roomType = roomForType.BlockFloor.Building.DormitoryId != null ? AccommodationType.Yurt : AccommodationType.Lojman;
                if (roomType != application.AccommodationType) return BadRequest("Seçilen oda başvurunun tesis türüyle uyuşmuyor.");
            }
        }
